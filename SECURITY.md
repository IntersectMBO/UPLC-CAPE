...
UPLC-CAPE is a benchmarking framework that publishes compiler-comparison reports. Its security posture rests on the following assumptions:

1. **Pull-request review is the trust boundary.** Every submission lands through a pull request reviewed by a maintainer. The review inspects the submission's structure, source, metadata, and artifacts against the project's templates and conventions before merging.
2. **`cape submission verify` is a developer-ergonomics tool, not a security gate.** It lets a submitter self-check that their artifact parses, runs against the scenario's tests, and produces a well-formed `metrics.json`. A green CI signal helps the reviewer; it does not replace the review.
3. **Submitters are trusted not to falsify their own benchmark results.** The framework is designed to compare honest submissions. Reports showing how a contributor _could_ publish numerically wrong but otherwise structurally valid measurements through a normal PR are not in scope; they fall under benchmark hygiene at review time.
...
- **Tooling exploitation via submitted artifacts**: A UPLC program or auxiliary submission file that, when processed by `measure`, `measure-preview`, the report generator, or any other framework tooling, causes crashes, RCE, sandbox escape, or other compromise of the host running CI.
- **Schema validation bypasses**: Issues that allow malformed `metadata.json` or `metrics.json` to pass automated validation in a way that compromises downstream tooling or the published report, beyond what a maintainer would catch in the diff.
...
- **Submitter dishonesty within the structural review surface**: A submitter who edits their own `metrics.json`, drops anomalous files into their submission directory, or otherwise produces a PR whose diff would be inspected by a maintainer falls under benchmark hygiene, not under this policy.
- **CI green check as a sole signal**: Demonstrations that `cape submission verify` returns success on an artifact the maintainer would reject at review do not by themselves constitute a vulnerability.
