...
- `cpu_units`, `memory_units`: Execution costs (with aggregations: max, sum, min, median — over the scenario's `measurements` tests)
- `excluded`: Diagnostic totals for evaluations excluded from aggregates (`checks` tests — attacks, malformed inputs, defensive checks — and pending measurements)
