...
import Plutarch.Internal.Term (compile, punsafeBuiltin, punsafeCoerce)
...
main :: IO ()
main =
  case compile mempty pfactorial of
    Left _ -> error "factorial compilation failed"
    Right (Script s) ->
      case runQuoteT $
        traverseOf progTerm unDeBruijnTerm $
          programMapNames fakeNameDeBruijn s of
        Left (_ :: FreeVariableError) -> error "factorial debruijn conversion failed"
