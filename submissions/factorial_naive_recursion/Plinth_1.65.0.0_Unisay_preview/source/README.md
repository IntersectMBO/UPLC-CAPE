...
This submission compiles `lib/Factorial.hs` from the Plinth source repository with the Plinth (plutus-tx-plugin) 1.65.0.0 line and the BuiltinCasing preview flag enabled. Requires `plutus-core >= 1.65.0.0` (not yet on mainnet).
...
The produced UPLC writes to `$CAPE_REPO/submissions/factorial_naive_recursion/Plinth_1.65.0.0_Unisay_preview/factorial_naive_recursion.uplc` and matches the UPLC in this submission.
