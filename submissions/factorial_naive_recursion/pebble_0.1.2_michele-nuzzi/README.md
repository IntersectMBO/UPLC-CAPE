...
```bash
pebble export --function-name factorial --entry source/factorial.pebble --output out/out.flat
```
...
```bash
rm factorial_naive_recursion.uplc # removes current template
aiken uplc decode out/out.flat >> factorial_naive_recursion.uplc
```
